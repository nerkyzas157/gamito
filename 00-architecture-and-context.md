...
| Recipe retrieval | Gamito (local embeddings + exact search) | Curated 14,619-recipe dataset + household-added custom recipes (§7.5) |
...
data files (CSV, lookups, built index ≈ 70 MB total) **are committed** — clone →
...
| Item | Estimate | Notes |
|---|---|---|
| Repo clone (with data + index) | ≈ 70–80 MB | CSV 26 MB + lookups + index ~45 MB; no build step after clone |
